...
# Install Julia
RUN wget https://julialang-s3.julialang.org/bin/linux/x64/1.6/julia-1.6.4-linux-x86_64.tar.gz && \
    mkdir "$JULIA_PATH" && \
    tar zxvf julia-1.6.4-linux-x86_64.tar.gz -C "$JULIA_PATH" --strip-components 1 && \
    rm julia-1.6.4-linux-x86_64.tar.gz # clean up
